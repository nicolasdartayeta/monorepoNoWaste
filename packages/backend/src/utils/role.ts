...
export async function deleteRole(role_id: string): Promise<boolean> {
  const result = await db.delete(role).where(eq(role.id, role_id)).returning();
...
export async function createPermission(
  ...
  return (await db.insert(permission).values(newPermission).returning())[0];
}
...
): Promise<NewRolePermission> {
  return (
    await db.insert(rolePermission).values(newRolePermission).returning()
  )[0];
}

export async function removeRolePermission(
  oldRolePermission: NewRolePermission,
): Promise<boolean> {
  const result = await db
